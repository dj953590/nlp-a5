...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        trun_len = int(torch.randint(low=4, high=int(self.block_size*7/8), size=(1,))[0])
        trunc_doc = document[:trun_len]
        
        masked_content_len = int(torch.randint(low=1, high=2*int(trun_len/4), size=(1,))[0])
        masked_content_index = int(torch.randint(low=0, high=trun_len - int(trun_len/4) + 1, size=(1,))[0])
        
        prefix = trunc_doc[:masked_content_index]
        suffix = trunc_doc[masked_content_index + masked_content_len:]
        masked_content = trunc_doc[masked_content_index : masked_content_index + masked_content_len]
