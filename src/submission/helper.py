...
def finetune(reading_params_path, finetune_corpus_path, pretrain_dataset, block_size, model, finetune_lr=6e-4, writer=None):
    ...
    if reading_params_path is not None:
        ...
        tconf = TrainerConfig(max_epochs=10, batch_size=256, learning_rate=finetune_lr, lr_decay=True, warmup_tokens=512*20, final_tokens=200*len(pretrain_dataset)*block_size, num_workers=4)
    else:
        tconf = TrainerConfig(max_epochs=75, batch_size=256, learning_rate=finetune_lr, lr_decay=True, warmup_tokens=512*20, final_tokens=200*len(pretrain_dataset)*block_size, num_workers=4)
    
    finetune_dataset = NameDataset(open(finetune_corpus_path, encoding='utf-8').read(), pretraining_dataset=pretrain_dataset)
...
def pretrain(pretrain_dataset, block_size, model, pretrain_lr=6e-3, writer=None):
    ...
    ### START CODE HERE
    tconf = TrainerConfig(max_epochs=650, batch_size=128, learning_rate=pretrain_lr, lr_decay=True, warmup_tokens=512*20, final_tokens=200*len(pretrain_dataset)*block_size, num_workers=4)
